...
export function ConsoleRegistrationForm() {
  ...
  return (
    <div>
      ...
      <form
        className={`${styles.form} ${step === 1 ? "" : styles.hidden}`}
        id="partner-account-form"
        onSubmit={nextStep}
      >
        ...
        <p className={styles.helper}>
          Gunakan minimal tiga kategori karakter: huruf besar, huruf kecil, angka, atau simbol.
        </p>
        {error && step === 1 ? (
      ...
      <form className={`${styles.form} ${step === 2 ? "" : styles.hidden}`} onSubmit={submit}>
        <div className={styles.twoColumns}>
          <label className={styles.field}>
            ...
            <input
              minLength={2}
              name="display_name"
              placeholder="Contoh: HomeCare Jakarta"
              required
            />
          </label>
          <label className={styles.field}>
            <span>Nama legal</span>
            <input minLength={2} name="legal_name" placeholder="PT / CV / nama usaha" required />
          </label>
        </div>
        <div className={styles.twoColumns}>
          <label className={styles.field}>
            <span>Jenis mitra</span>
            <select defaultValue="service_provider" name="partner_type">
              <option value="service_provider">Penyedia home service</option>
              <option value="company">Perusahaan</option>
              <option value="store">Toko / merchant</option>
              <option value="clinic">Klinik</option>
              <option value="individual">Profesional individu</option>
            </select>
          </label>
          <label className={styles.field}>
            <span>Nomor telepon</span>
            <input minLength={7} name="contact_phone" placeholder="08xxxxxxxxxx" required />
          </label>
        </div>
        <label className={styles.field}>
          <span>Alamat operasional</span>
          <input minLength={5} name="address_line" placeholder="Jalan, nomor, kecamatan" required />
        </label>
        <div className={styles.threeColumns}>
          <label className={styles.field}>
            <span>Kota</span>
            <input minLength={2} name="city" required />
          </label>
          <label className={styles.field}>
            <span>Provinsi</span>
            <input minLength={2} name="province" required />
          </label>
          <label className={styles.field}>
            <span>Kode pos</span>
        ...
        <label className={styles.field}>
          ...
          <textarea
            maxLength={2000}
            name="description"
            placeholder="Jelaskan produk atau layanan utama."
            rows={3}
          />
        </label>
        <label className={styles.checkbox}>
          <input required type="checkbox" />{" "}
          <span>Saya menyetujui proses verifikasi dan kebijakan kemitraan Yoru.</span>
        </label>
        {error && step === 2 ? (
          <p className={styles.error} role="alert">
            {error}
        ...
        <div className={styles.actionRow}>
          ...
          <button className={styles.submit} disabled={pending} type="submit">
            {pending ? "Membuat workspace..." : "Daftar sebagai mitra"}
          </button>
        </div>
      </form>
      <p className={styles.switchText}>
        Sudah punya akun? <Link href="/login">Masuk ke dashboard</Link>
      </p>
    </div>
  );
}
