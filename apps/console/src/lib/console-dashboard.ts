...
export function resolveConsoleRole(session: ConsoleSessionShape): ConsoleRole {
  const role = resolveConsoleAccess(session);
  if (!role) throw new Error("CONSOLE_ACCESS_DENIED");
  return role;
}
...
export const superAdminDemoProfile = {
  organization: "Yoru Indonesia",
  environment: "Development demo",
  dataUpdatedAt: "02 Agu 2026 · 21:45 WIB",
  openVerifications: 17,
  openDisputes: 4,
} as const;
