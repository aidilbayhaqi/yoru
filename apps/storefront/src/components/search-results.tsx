...
export function SearchResults({ query, mode = "query" }: { query: string; mode?: SearchMode }) {
  ...
  const serviceResults = services.filter((service) =>
    [service.name, service.category, service.partner, service.serviceArea, service.description, ...service.tags]
      .join(" ")
      .toLowerCase()
      .includes(normalized),
  );
  const count = productResults.length + serviceResults.length;
  const modeLabel = mode === "ai" ? "AI intent search" : mode === "image" ? "Visual search preview" : "Query search";
    ...
    <main className="search-page v5-search-page">
      ...
      {mode === "ai" ? (
        <section className="search-intent-card">
          <span><Icon name="wand" width="22" /></span>
          <div>
            <strong>How Yoru interpreted your request</strong>
            <p>Keyword, category, partner, description, tag, and service-area matching for “{query}”. Transaction facts still come from product and booking APIs.</p>
          </div>
        </section>
      ) : null}
      ...
      {mode === "image" ? (
        <section className="search-intent-card search-intent-card--image">
          ...
          <div>
            ...
            <p>These results use the visual category intent. Production similarity requires an image-embedding endpoint, moderation, and vector retrieval.</p>
          </div>
        </section>
      ) : null}
      ...
      {productResults.length > 0 ? (
        <section className="search-section">
          <div className="section-heading-row">
            <div><p className="section-eyebrow">Commerce</p><h2>Products</h2></div>
            <span>{productResults.length} results</span>
          </div>
          <div className="catalog-grid">{productResults.map((product) => <ProductCard key={product.id} product={product} />)}</div>
        </section>
      ) : null}
      ...
      {serviceResults.length > 0 ? (
        <section className="search-section">
          <div className="section-heading-row">
            <div><p className="section-eyebrow">At home</p><h2>Home services</h2></div>
            <span>{serviceResults.length} results</span>
          </div>
          <div className="catalog-grid">{serviceResults.map((service) => <ServiceCard key={service.id} service={service} />)}</div>
        </section>
      ) : null}
      ...
      {count === 0 ? (
        <section className="empty-state inline-empty-state v5-empty-state">
          ...
          <div><Link className="primary-button" href="/products">Browse products</Link><Link className="secondary-button" href="/services">Browse services</Link></div>
        </section>
      ) : null}
    </main>
  );
