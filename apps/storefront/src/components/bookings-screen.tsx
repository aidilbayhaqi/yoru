  ...
  return (
    <main className="customer-list-page">
      <div className="page-heading">
        ...
        <p>Pantau jadwal, profesional, OTP, dan status layanan di rumah.</p>
      </div>
      {state.bookings.length === 0 ? (
        <section className="empty-state inline-empty-state">
          <span className="empty-state__icon">
            <Icon name="calendar" width="32" />
          </span>
          <h2>Belum ada booking.</h2>
          <p>Pilih layanan dan jadwal yang paling sesuai.</p>
          ...
          {state.bookings.map((booking) => (
            <article key={booking.id}>
              <div className="record-header">
                ...
                <span className={`record-status status-${booking.status}`}>
                  {labels[booking.status]}
                </span>
              </div>
              <div className="booking-record-body">
                <div>
