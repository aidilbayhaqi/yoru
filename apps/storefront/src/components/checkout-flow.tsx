...
export function CheckoutFlow() {
  ...
  const checkoutLines = buyNowMode && state.buyNow ? [state.buyNow] : state.cart;
    ...
    <main className="checkout-shell">
      <section className="checkout-main">
        <div className="page-heading compact-heading">
          <p className="section-eyebrow">Secure product checkout</p>
          ...
          <form className="checkout-card form-grid" onSubmit={submitAddress}>
            <div className="checkout-card__heading form-span-2">
              ...
              <div>
                ...
                <p>Pastikan nomor telepon aktif untuk koordinasi kurir.</p>
              </div>
            </div>
            <label>
              Nama penerima
            ...
            <label className="form-span-2">
              Catatan kurir
              <input defaultValue={address.notes} name="notes" placeholder="Opsional" />
            </label>
            <button className="primary-button form-next-button" type="submit">
              Pilih pengiriman
        ...
        {step === 1 ? (
          <section className="checkout-card">
            <div className="checkout-card__heading">
              <span className="checkout-icon">
                <Icon name="truck" width="21" />
              </span>
              <div>
                <h2>Metode pengiriman</h2>
                <p>Estimasi final mengikuti alamat dan partner fulfillment.</p>
              </div>
            </div>
            <div className="selection-list">
              {[
                ...
                <button
                  className={deliveryMethod === id ? "is-selected" : ""}
                  key={String(id)}
                  onClick={() => setDeliveryMethod(String(id))}
                  type="button"
                >
                  ...
                  <strong>{Number(price) === 0 ? "Gratis" : formatMoney(Number(price))}</strong>
                </button>
              ))}
            </div>
        ...
        {step === 2 ? (
          <section className="checkout-card">
            <div className="checkout-card__heading">
              ...
              <div>
                <h2>Pembayaran</h2>
                <p>Demo frontend menandai pembayaran berhasil agar seluruh flow dapat diuji.</p>
              </div>
            </div>
            ...
            <div className="checkout-navigation">
              ...
              <button
                ...
                type="button"
              >
                {pending ? "Memproses..." : "Bayar dan buat pesanan"}
      ...
      <aside className="summary-card checkout-summary">
        <p className="summary-kicker">{buyNowMode ? "Buy now summary" : "Order summary"}</p>
          ...
          {checkoutLines.map((line) => {
            ...
            return (
              <div key={line.lineId}>
                ...
                <strong>{formatMoney(variant.priceMinor * line.quantity)}</strong>
              </div>
            );
          })}
        </div>
        ...
        <div className="summary-row">
          ...
          <span>{formatMoney(totals.shippingMinor)}</span>
        </div>
        <div className="summary-row">
          <span>Biaya layanan</span>
          <span>{formatMoney(totals.serviceFeeMinor)}</span>
        </div>
        ...
        <div className="summary-note">
          ...
          <span>Total production harus berasal dari quote server yang belum kedaluwarsa.</span>
        </div>
      </aside>
    </main>
  );
