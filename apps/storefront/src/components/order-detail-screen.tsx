  ...
  return (
    <main className="record-detail-page">
      <div className="record-detail-heading">
        ...
        <span className={`record-status status-${order.status}`}>Diproses</span>
      </div>
                ...
                return (
                  <div key={line.lineId}>
                    <div>
                      ...
                      <span>
                        {variant?.name} · {line.quantity} item
                      </span>
                    </div>
                    <strong>{formatMoney((variant?.priceMinor ?? 0) * line.quantity)}</strong>
                  </div>
                );
              })}
            </div>
        ...
        <aside className="record-detail-side">
          ...
          <article className="record-card">
            ...
            <div className="summary-row">
              ...
              <span>{formatMoney(order.serviceFeeMinor)}</span>
            </div>
            <div className="summary-total">
              <span>Total</span>
              <strong>{formatMoney(order.totalMinor)}</strong>
            </div>
            <p className="record-payment-method">{order.paymentMethod.replaceAll("_", " ")}</p>
          </article>
        </aside>
      </div>
    </main>
