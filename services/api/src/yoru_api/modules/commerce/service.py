...
class CommerceService:
    ...
    async def create_quote(self, *, actor: Actor, cart_id: uuid.UUID | None) -> Quote:
        cart = (
    ...
    async def confirm_checkout(
        ...
        *,
        ...
        payment = await self._repository.add_payment_intent(
            order_id=order.id,
            customer_id=actor.user_id,
            partner_id=order.partner_id,
            provider=provider,
    ...
    async def process_webhook(
        ...
        *,
        ...
        existing_event = await self._repository.get_payment_event(
            provider=provider, provider_event_id=payload.provider_event_id
        )
        ...
        if payload.status == "succeeded":
            ...
            for reservation in await self._repository.list_reservations(order.id, for_update=True):
                ...
                if inventory is None or inventory.reserved < reservation.quantity:
                    raise AppError(409, "INVENTORY_RESERVATION_CONFLICT", "Inventory reservation is invalid")
            ...
            if order.state == "pending_payment":
                ensure_order_transition(order.state, "cancelled")
                order.state = "cancelled"
                order.payment_status = "failed" if target == "failed" else "cancelled"
                order.fulfillment_status = "cancelled"
                order.cancelled_at = now
        ...
        self._repository.add_outbox(
            ...
            payload={"payment_intent_id": str(payment.id), "order_id": str(order.id)},
        )
        await self._repository.commit()
        return payment
