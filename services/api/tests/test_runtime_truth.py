...
def test_metadata_reports_runtime_truth(client: TestClient) -> None:
...
def test_required_routes_are_present_in_openapi(app: FastAPI) -> None:
    schema = app.openapi()
    schema_routes = {
        (method.upper(), path)
    ...
    assert REQUIRED_ROUTES <= schema_routes
